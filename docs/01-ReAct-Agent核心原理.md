...
```python
graph.with_config({"recursion_limit": 30})
```

`recursion_limit` 是 LangGraph 的安全阀。它限制了一个回合中 LLM ↔ Tool 往返的最大次数（默认 25，本项目设为 30）。如果 Agent 陷入循环（反复调用同一个工具），超过限制后 LangGraph 会抛出 `GraphRecursionError`，阻止无限消耗 token。
...
- **循环**：`recursion_limit=30` + `AGENTS.md` 中明确的"禁止重复调用"规则
