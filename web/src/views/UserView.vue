<template>
  <div class="user-view">
    <div class="header">
      <h2>用户 <span class="subtitle">USER</span></h2>
...
async function loadContent() {
  loading.value = true
  try {
    const res = await api.getPersona('user')
    content.value = res.content
  } catch (e: any) {
    console.error('加载用户描述失败', e)
  } finally {
    loading.value = false
  }
}
...
async function saveContent() {
  saving.value = true
  try {
    await api.updatePersona('user', content.value)
    saved.value = true
    setTimeout(() => { saved.value = false }, 2000)
  } catch (e: any) {
    console.error('保存用户描述失败', e)
  } finally {
    saving.value = false
  }
}
